...
BIGQUERY_CONFIG_ARGS = [
    "project_id",
    "dataset_id",
    "table_id",  # 'schema_storage_location'
]
...
def run(request):
    ...
    get_orders_result = get_orders(
        ...
        start_date=inputs_dict["start_date"],
        end_date=inputs_dict["end_date"],
    )
...
def get_orders(
    upgates_login, upgates_api_key, upgates_api_url, start_date=None, end_date=None
):
    """
    ...
    try:
        ...
        gcp_log(
            "INFO", "Fetching orders...", dict(start_date=start_date, end_date=end_date)
        )

        try:
            # Make an initial API call to get the number of pages
            response = requests.get(upgates_api_url, auth=auth_header, params=params)
        ...
        for page in range(1, number_of_pages + 1):
            ...
            if page % 10 == 0:
                gcp_log(
                    ...
                    dict(page=page, number_of_pages=number_of_pages),
                )
...
def check_request_args(request):
    ...
    gcp_log(
        "INFO", "Checking the request args...", dict(job_phase="check_request_args")
    )
    if not request_json:
        return gcp_log(
            "ERROR",
            "The body of the request is missing. Please enter a body with valid values for: {FUNCTION_ARGS}.",
            dict(job_phase=job_phase, job_phase_detail="request_body"),
        )
        ...
        if arg in request_json and request_json[arg] is not None:
            ...
            gcp_log(
                ...
                dict(
                    ...
                    arg_details={"name": arg, "value": DEFAULTS_ARG_VALUES[arg]},
                ),
            )
        ...
        if arg in request_json and request_json[arg] is not None:
            ...
            if arg == "bigquery_config":
                for bq_arg in BIGQUERY_CONFIG_ARGS:
                    if (
                        bq_arg in request_json[arg]
                        and request_json[arg][bq_arg] is not None
                    ):
...
def get_bq_schema():
    ...
    schema = [
        ...
        bigquery.SchemaField("statistics_yn", "BOOLEAN"),
